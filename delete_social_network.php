...
$result  = array(DataQueries::GetUser($username));
...
//	$networkResult = $db->Query($networkQuery_escaped);
	...
	$networkResult =  array(DataQueries::GetSocialKey($username));

	while ($row = $networkResult[0][0]) {
...
//
		DataQueries::DeleteNetwork($hash,$username);

		//Dropping DataBASE !!!
//		$drop_db = "DROP DATABASE " . $hash;
//		$db->Query($drop_db);

		DataQueries::DeleteDB($hash);
